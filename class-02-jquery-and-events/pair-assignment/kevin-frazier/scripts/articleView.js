...
articleView.handleAuthorFilter = function() {
  ...
  $('#author-filter').on('change', function() {
    if ($(this).val()) {
      ...
      $articles.hide();
      console.log($articles.has('address a:contains("'+$(this).val() + '")'));
      $articles.has('address a:contains("'+$(this).val() + '")').fadeIn();
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      ...
      var $articles = $('article');
      $articles.hide();
      $articles.filter('[data-category="' + $(this).val() + '"]').fadeIn();
    } else {
      $('article').each(function() {
        if (!$(this).hasClass('template')) {
          $(this).fadeIn();
...
articleView.handleMainNav = function() {
  ...
  $('.main-nav').on('click', function(event){
    ...
    var sectionId = $('.tab').has(' a:contains("'+text + '")').attr('data-content');
    console.log($('.tab').has(' a:contains("'+text + '")').attr('data-content'));
    $('#' + sectionId).fadeIn();
  });
